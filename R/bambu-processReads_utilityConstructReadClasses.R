...
#' @inheritParams bambu
#' @noRd
isore.constructReadClasses <- function(readGrgList, unlisted_junctions, preset = "unstranded_cDNA",
                                       uniqueJunctions, runName = "sample1",
                                       annotations, stranded = FALSE, verbose = FALSE, referenceTss = NULL, rc.startEndGroupDist = 0) {
    ...
    mcols(reads.singleExon)$sampleID <- mcols(readGrgList[
        elementNROWS(readGrgList) == 1])$sampleID    
    ...
    if(!is.null(uniqueJunctions)){
        exonsByRC.spliced <- constructSplicedReadClasses(
            ...
            stranded = stranded, annotations, referenceTss = referenceTss, rc.startEndGroupDist = rc.startEndGroupDist)}
...
#' @noRd
constructSplicedReadClasses <- function(uniqueJunctions, unlisted_junctions, preset = "unstranded_cDNA",
                                        readGrgList, annotations, stranded = FALSE, referenceTss = NULL, rc.startEndGroupDist = 0) {
    options(scipen = 999)
    ...
    readTable <- createReadTable(start(unlisted_junctions), 
        end(unlisted_junctions), mcols(unlisted_junctions)$id, readGrgList, preset = preset, 
        readStrand, readConfidence, annotations, referenceTss = referenceTss, rc.startEndGroupDist)
...
#'     row_number .groups
#' @noRd
createReadTable <- function(unlisted_junctions_start, unlisted_junctions_end, preset = "unstranded_cDNA",
    unlisted_junctions_id, readGrgList,readStrand, readConfidence, annotations, referenceTss = NULL, rc.startEndGroupDist = 0) {
    ...
    tssList <- prepareTssFromReads(readTable)
    tesList <- prepareTesFromReads(readTable)
    ...
    readTable <- splitReadClassByStartEnd(readTable, annoTable, rc.startEndGroupDist, alternativeStartEnd = F)
    ...
    readTable <- assignRegionIdToRc(readTable, annoTable, rc.startEndGroupDist)
    return(readTable)
}
...
prepareTssFromReads <- function(readTable, max_dist = 100, min_reads = 10){
  collapsed_tss <- readTable %>%
    mutate(tss = ifelse(strand != "-", start, end)) %>%
    select(seqnames = chr, start = tss, strand) %>%
    ...
    summarise(
      start = start[which.max(N)],   # choose the TSS with the highest N
      N = sum(N),                    # sum N across the cluster
      .groups = "drop"
    ) %>%
    filter(N >= min_reads)
  referenceTSS <- GRanges(seqnames = collapsed_tss$seqnames,
                          ranges = IRanges(start = collapsed_tss$start, width = 1),
...
prepareTesFromReads <- function(readTable, max_dist = 100, min_reads = 10){
  collapsed_tes <- readTable %>%
    mutate(tes = ifelse(strand != "-", end, start)) %>%
    select(seqnames = chr, end = tes, strand) %>%
    group_by(seqnames, end, strand) %>%
    summarise(N = n(), .groups = "drop") %>%
    filter(N > min_reads) %>%
    arrange(seqnames, end, strand) %>%
    group_by(seqnames, strand) %>%
...
splitReadClassByStartEnd <- function(readTable, annoTable, rc.startEndGroupDist = 0, alternativeStartEnd = FALSE){
  ...
  readTable <- readTable %>% 
    ...
    mutate(firstExonGroup = ifelse(
      strand != "-",
      safeFind(start, start[is.na(readId)], window = rc.startEndGroupDist),
      safeFind(-end, -end[is.na(readId)], window= rc.startEndGroupDist)
    )) %>% 
    ungroup() %>%
...
safeFind <- function(x, ref, window, start_end = "start"){
  if (length(ref) == 0) {
...
assignRegionIdToRc <- function(readTable, annoTable, rc.startEndGroupDist = 0){
  readTable <- bind_rows(readTable, annoTable)
  readTable <- readTable %>% 
    group_by(chr, strand) %>% 
    mutate(startRegionId = ifelse(strand != "-", 
                                   findInterval(start,sort(start[is.na(readClassId)]) - rc.startEndGroupDist),
                                   findInterval(-end,sort(-end[is.na(readClassId)]) - rc.startEndGroupDist))) %>% ungroup() %>%
    group_by(chr, strand) %>% 
    mutate(endRegionId = ifelse(strand != "-", 
                                  findInterval(-end,sort(-end[is.na(readClassId)]) - rc.startEndGroupDist),
                                  findInterval(start,sort(start[is.na(readClassId)]) - rc.startEndGroupDist))) %>% ungroup() %>% 
...
constructUnsplicedReadClasses <- function(reads.singleExon, annotations, 
    ...
    counts = as.data.frame(reads.singleExon) %>% 
        ...
        sampleID = mcols(reads.singleExon)$sampleID) %>% 
        group_by(seqnames,start,end,strand) %>% 
        mutate(counts=n(), id = list(id), sampleID = list(sampleID)) %>% 
        ungroup() %>%
        as.data.frame()
    reads.singleExon = GRanges(counts)
...
getUnsplicedReadClassByReference <- function(granges, grangesReference,
    ...
    hitsDF <- hitsDF %>% dplyr::select(queryHits, chr, start, end, strand) %>%
      ...
      mutate(alignmentStrand = as.character(strand(granges))[queryHits]=="+",
             ...
             sampleID = mcols(granges[queryHits])$sampleID)
    hitsDF <- hitsDF %>% 
        ...
        summarise(start = start[1], end = end[1], 
            ...
            readIds = list(unlist(readId)), sampleIDs = list(unlist(sampleID))) %>% 
    ...
    hitsDF <- dplyr::select(hitsDF, chr.rc = chr, strand.rc = strand,
        ...
        readCount.posStrand, readIds, sampleIDs)
